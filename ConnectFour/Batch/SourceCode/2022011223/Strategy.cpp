...
/*
	...
	input:
		...
		noX, noY : 棋盘上的不可落子点(注:框架已经替你处理了不可落子点，也就是说如果某一步
...
extern "C" Point* getPoint(const int M, const int N, const int* _top,
	...
	*/
	...
	UCT uct = UCT(M, N, Point(noX, noY), board, top);
	UCTNode *result = uct.UCTSearch();
	x = result->state.position.x;
	y = result->state.position.y;
